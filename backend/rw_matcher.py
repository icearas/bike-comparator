"""
RW_MATCHER — dopasowywanie CR ↔ rowerowy.com przez AI
Wyniki zapisywane do data/rw_matched.csv (Option C — bez refaktoru DB)
"""
...
OUT_PATH = Path(__file__).parent.parent / "data" / "rw_matched.csv"

_last_call = 0.0
_rate_lock = asyncio.Lock()


async def are_same_product(name_cr: str, name_rw: str, category: str) -> tuple[bool, float]:
...
async def rate_limited_call(name_cr: str, name_rw: str, category: str) -> tuple[bool, float]:
    global _last_call
    ...
    return await are_same_product(name_cr, name_rw, category)


async def match_rw(limit: int = 300):
    start = time.time()
    print(f"🚀 RW MATCHER | Limit: {limit} | Start: {time.strftime('%H:%M:%S')}")

    db = SessionLocal()
    try:
        rules = load_filter_rules(db)
        cr_products = db.query(Product).filter_by(shop="centrumrowerowe.pl").all()
        rw_products = db.query(Product).filter_by(shop="rowerowy.com").all()
    finally:
        db.close()

    cr_main = [p for p in cr_products if is_main_product(p.name, p.category, rules, p.url or "")]
    rw_main = [p for p in rw_products if is_main_product(p.name, p.category, rules, p.url or "")]

    print(f"CR po filtrowaniu: {len(cr_main)} | RW po filtrowaniu: {len(rw_main)}")

    rw_by_brand_cat: dict[tuple, list] = {}
    for rw in rw_main:
        brand = extract_brand(rw.name, rw.url)
        key = (brand, rw.category)
        rw_by_brand_cat.setdefault(key, []).append(rw)
    ...
    async def process_cr(i: int, cr: Product):
        ...
        async with semaphore:
            ...
            rw_category = CATEGORY_MAP.get(cr.category, cr.category)

            if cr.category in ("widelce", "dampery"):
                candidates = (
                    rw_by_brand_cat.get((cr_brand, "widelce"), []) +
                    rw_by_brand_cat.get((cr_brand, "dampery"), [])
                )
                cr_grade = extract_suspension_grade(cr.name)
                if cr_grade:
                    grade_filtered = [rw for rw in candidates if extract_suspension_grade(rw.name) == cr_grade]
                    if grade_filtered:
                        candidates = grade_filtered
                    else:
                        print(f"❌ [{i+1}] {cr.name[:50]} - RW nie ma grade '{cr_grade}'")
                        return
            else:
                candidates = rw_by_brand_cat.get((cr_brand, rw_category), [])
            ...
            if cr_models:
                filtered = [rw for rw in candidates if any(m in rw.name.upper() for m in cr_models)]
                if filtered:
            ...
            for rw in candidates:
                is_same, confidence = await rate_limited_call(cr.name, rw.name, cr.category)
            ...
            if best_match:
                async with results_lock:
                    results.append({
                        ...
                        "cr_url": cr.url or "",
                        "rw_name": best_match.name,
                        "rw_price_pln": best_match.price,
                        "rw_url": best_match.url or "",
