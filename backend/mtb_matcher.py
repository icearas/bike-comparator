...
async def are_same_product(name_cr: str, name_mtb: str, category: str) -> tuple[bool, float]:
...
async def rate_limited_call(name_cr: str, name_mtb: str, category: str) -> tuple[bool, float]:
    global _last_call
    ...
    return await are_same_product(name_cr, name_mtb, category)


async def match_mtb(limit: int = 300):
    start = time.time()
    ...
    async def process_cr(i: int, cr: SimpleNamespace):
        ...
        async with semaphore:
            ...
            mtb_category = CATEGORY_MAP.get(cr.category, cr.category)

            if cr.category in ("widelce", "dampery"):
                candidates = (
                    mtb_by_brand_cat.get((cr_brand, "widelce"), []) +
                    mtb_by_brand_cat.get((cr_brand, "dampery"), [])
                )
                ...
                if cr_grade:
                    ...
                    if grade_filtered:
                        ...
                        print(f"❌ [{i+1}] {cr.name[:50]} - MTB nie ma grade '{cr_grade}'")
                        return
            else:
                candidates = mtb_by_brand_cat.get((cr_brand, mtb_category), [])
            ...
            best_match = None
            best_confidence = 0.0
            for mtb in candidates:
                is_same, confidence = await rate_limited_call(cr.name, mtb.name, cr.category)
